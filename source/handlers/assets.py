...
class FSMCryptoAsset(StatesGroup):
    crypto_name = State()
    value = State()
...
async def get_crypto_price_state(
    mess: types.Message
):
...
async def add_crypto_asset_state_starter(
    ...
    await FSMCryptoAsset.crypto_name.set()
    await mess.reply('Введите полное название криптовалюты которую '
                     'хотите добавить в портфель(например: bitcoin)')


async def save_crypto_asset_name(
    mess: types.Message,
    state: FSMContext
):
    await state.update_data(crypto_name=mess.text.lower())

    await FSMCryptoAsset.value.set()
    await mess.reply(f'Введите количество {mess.text.lower()}')


async def save_crypto_asset_value(
    mess: types.Message,
    state: FSMContext
):
